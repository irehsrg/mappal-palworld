...
| C3 | Client-side only. No backend, no upload. **Amended 2026-07-21 (Alex's call, deliberate):** the optional community gallery (docs/GALLERY.md) uses a hosted backend (Supabase) for auth + storage. The *editor* remains fully client-side — parsing, editing, and export never touch a server; a blueprint leaves the machine only when the user explicitly clicks Publish/Save-to-account. With gallery env vars absent the app runs identically to pure-C3. **Amended 2026-07-23:** anonymous usage analytics (PostHog: feature-usage counts, error capture — see src/analytics.ts for the counts-and-flags-only contract) run alongside Vercel Analytics. Blueprint parsing, editing, and export still never touch a server. | Blueprints are user data. Also makes it free to host. |
